...
var URL = 'http://twitterstreamhw2v3-env.elasticbeanstalk.com';
...
function initialize() {
  ...
  // Setup Google Map
  var myLatlng = new google.maps.LatLng(40.463,-3.749);
  ...
  //if(io !== undefined) {
    ...
    var socket = io.connect(URL, { port: 8081 });
    ...
    socket.on('add:marker', function (data) {
      ...
      console.log('Recieved marker to plot on map: ' + data);

      var marker_obj = JSON.parse(data);
      
      //Add tweet to the heat map array.
      var tweetLocation = new google.maps.LatLng(marker_obj.lat,marker_obj.lng);
      liveTweets.push(tweetLocation);

      //Flash a dot onto the map quickly

      if (marker_obj.sentiment == "positive") {
        var image = "css/green.png";
        console.log("postive sentiment recieved");
      }

      else if (marker_obj.sentiment == "negative") {
        var image = "css/red.png";
        console.log("negative sentiment recieved");
      }

      else if (marker_obj.sentiment == "neutral") {
